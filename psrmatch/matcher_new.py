...
class Matcher(object):
    ...
    def query_search_tree(self, source, dm):
        """
        ...
        Returns
        -------
        chord_spatial, idx: ~np.array of float, ~np.array of int
            The cartesian chord lengths or distances and indices of the nearest neighbors.
        """

        max_chord_spatial = 2 * np.sin(0.5 * self.dist_thresh * np.pi / 180.0)
        self.__log.debug("Maximum spatial chord length: {0}".format(max_chord_spatial))

        # 5 per cent for 1000 dm pulsar
        max_dm_diff = 50.0
        max_chord = np.sqrt(max_chord_spatial**2 + max_dm_diff**2)

        chord, idx = self.__tree.query(
            x=[source.cartesian.x, source.cartesian.y, source.cartesian.z, dm],
            p=2,
        ...
        chord_spatial = np.sqrt(chord**2 - (dm - self.catalogue[idx]["dm"]) ** 2)

        self.__log.info("Nearest neighbors:")
        for c, cs, i in zip(chord, chord_spatial, idx):
            info_str = "{0:.3f} {1:.3f}: {2:10} {3:17} {4:17} {5:.3f}".format(
                c,
                cs,
    ...
    def find_matches(self, source, dm):
        ...
        chord, idx = self.query_search_tree(source, dm)
