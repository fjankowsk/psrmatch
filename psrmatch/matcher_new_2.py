...
class Matcher(object):
    ...
    def find_matches(self, source, dm):
        ...
        dm_diff = np.abs(dm - self.catalogue[idx]["dm"]) / float(dm)
        sort_idx = np.argsort(dm_diff)

        chord = chord[sort_idx]
        idx = idx[sort_idx]

        match = None

        for d, c, i in zip(dm_diff, chord, idx):
            ...
            if sep < self.dist_thresh and d < self.dm_thresh:
