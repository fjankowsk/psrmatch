...
pulsar = {"ra": "14:53:32.665", "dec": "-64:13:16.00", "dm": 71.248}

pcoord = SkyCoord(
    ra=pulsar["ra"], dec=pulsar["dec"], frame="icrs", unit=(units.hourangle, units.deg)
)
...
print("{0:<15} {1:<10} {2:<20}".format("Separation", "DM offset", "Matches"))

for i in range(len(df.index)):
    item = df.loc[i]
    coord = SkyCoord(
        ra=item["ra"], dec=item["dec"], frame="icrs", unit=(units.hourangle, units.deg)
    )
...
ax.scatter(pcoord.ra.deg, pcoord.dec.deg, s=10, color="tab:red", marker="*", zorder=8)
